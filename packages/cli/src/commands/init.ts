    ...
    try {
      ...
      if (!fs.existsSync(claudeMdPath) || opts.force) {
        const pmAgentInstructions = [
          ...
          '- Commit messages must only contain the authored content — no tool signatures',
          '- This applies to every commit, merge commit, and rebase operation',
          '- Verified: run `git log --format="%B" | grep -i "co-authored-by"` should return nothing',
          '',
          '## Core Principles',
          '',
          ...
          '- No AI attribution in commit messages — see Git Commit Rules section',
          '',
        ];
